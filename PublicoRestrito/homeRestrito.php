...
// Buscar o nome do funcionário logado
$sql = <<<SQL
SELECT p.Nome, m.Codigo AS MedicoCodigo
FROM Pessoa p
LEFT JOIN Medico m ON m.Codigo = p.Codigo
WHERE p.Email = ?
SQL;
...
if ($user) {
  // Guarda o nome em sessão para reutilizar
  $_SESSION['nome'] = $user['Nome'];

  // Se ele for médico, marcar flag
  if ($user['MedicoCodigo'] !== null) {
...
<body>
  ...
  <main>
    <h2>Área Restrita</h2>
    <h3>Bem Vindo, <?php echo htmlspecialchars($_SESSION['nome'] ?? $_SESSION['user']); ?>! Aqui você consegue:</h3>
    <ul>
